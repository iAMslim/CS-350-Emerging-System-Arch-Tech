...
class TemperatureMachine(StateMachine):
    ...
    def on_exit_heat(self):
        ##ensure we stop any pulsing when leaving heat
        redLight.off()
    ...
    def on_exit_cool(self):
        ## Ensure pulsing stops when leaving cool
        blueLight.off()
    ...
    def processTempStateButton(self):
        ...
        self.cycle()
        # Refresh LEDs after state change
        self.updateLights()
    ...
    def processTempIncButton(self):
        ...
        ## Increase setpoint by 1F
        self.setPoint = self.setPoint + 1
        # Update LEDs to reflect new relationships to temperature
        self.updateLights()
    ...
    def updateLights(self):
        ...
        if state == "heat":
            #fade if current temp is below setpoint; otherwise solid
        ...
        if state == "cool":
             # Fade if current temp is above setpoint; otherwise solid
             if temp > self.setPoint:
                 redLight.off()
                 blueLight.pulse()
             else:
    ...
    def setupSerialOutput(self):
        ## Comma delimited: state, currentTemp, setPoint
        state = self.current_state.id
        current_temp_f = floor(self.getFahrenheit())
        output = f"{state},{current_temp_f},{self.setPoint}"
        return output
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            ## Line 1 be date + time (fit 16 chars)
            ## format: MM/DD HH:MM:DD (14 chars) then newline
            lcd_line_1 = current_time.strftime("%m/%d %H:%M:%S").ljust(16)[:16] + "\n"

            ## Setup Display Line 2
            if(altCounter < 6):
                ## current temp - state + setpoint
                temp_f = self.getFahrenheit()
                # Example: "Temp: 72.4F"
                lcd_line_2 = f"Temp:{temp_f:6.1f}F".ljust(16)[:16]
                altCounter = altCounter + 1
            else:
                ## Example: "heat SP:72F"
                state = self.current_state.id
                lcd_line_2 = f"{state} SP:{self.setPoint}F".ljust(16)[:16]
                altCounter = altCounter + 1
